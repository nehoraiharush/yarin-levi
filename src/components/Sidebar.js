...
const Sidebar = ({ isOpen, setOpen, trainer }) => {
  ...
  const managerConeected =
    localStorage.getItem(USERNAME) === MANAGERNAME ? true : false;
          ...
          <Nav defaultActiveKey={`../home`} className='nav'>
            {userConnected ? (
                ...
                {trainer ? (
                  <>
                    ...
                    <Link
                      className='nav-link'
                      to={`../${
                        managerConeected ? 'editor' : 'training-info'
                      }/${trainer.id}/trainingPlan`}
                    >
                      תכנית אימונים
                    </Link>
                    <Link
                      ...
                      to={`../${
                        managerConeected ? 'editor' : 'training-info'
                      }/${trainer.id}/nutrition`}
                    >
                      תפריט תזונה
                    </Link>
                    <Link
                      className='nav-link'
                      to={`../editor/${trainer.id}/progress`}
                    >
                      מעקב התקדמות
                    </Link>
                  </>
              ...
              <div>
                ...
                <Nav.Link
                  href={`https://wa.me/972584560107/?text=${encodeURIComponent(
                    MESSAGE
                  )}`}
                  target='_blank'
                >
                  התחל את השינוי <strong>שלך</strong>
                </Nav.Link>
              </div>
            )}
          </Nav>
